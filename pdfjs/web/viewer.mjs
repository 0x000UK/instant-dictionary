...
function translateElement(element, translation) {
  ...
  if (typeof value === "string") {
    if (element.localName === "title" && element.namespaceURI === "http://www.w3.org/1999/xhtml") {
      ...
      /* AMO SEC: DOMParser replacement for innerHTML */
      const parsedDoc = new DOMParser().parseFromString(value, "text/html");
      templateElement.content.replaceChildren(...parsedDoc.body.childNodes);
...
class GenericScripting {
  constructor(sandboxBundleSrc) {
    this._ready = new Promise((resolve, reject) => {
      const sandbox = Promise.reject(new Error("AMO SEC: Scripting sandbox blocked"));
...
const PDFViewerApplication = {
  ...
  async _parseHashParams() {
    ...
    const loadPDFBug = async () => {
      ...
      throw new Error("AMO SEC: PDFBug import blocked");
      this._PDFBug = PDFBug;
    };
    if (params.get("disableworker") === "true") {
      try {
        GlobalWorkerOptions.workerSrc ||= AppOptions.get("workerSrc");
        throw new Error("AMO SEC: disableworker import blocked");
...
PDFPrintServiceFactory.initGlobals(PDFViewerApplication);
{
  ...
  var validateFileURL = function (file) {
    ...
    /* [Instant Dictionary Enterprise] Bypassed origin lock to allow extension to render web PDFs */
  };
